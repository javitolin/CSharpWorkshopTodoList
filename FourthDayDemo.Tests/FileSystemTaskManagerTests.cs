...
namespace ToDoConsoleApp.Tests
{
    ...
    public class FileSystemFixture : IDisposable
    {
        ...
        public FileSystemFixture()
        {
            ...
            fileMock.Setup(f => f.ReadAllTextAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(() => fileContent);
            fileMock.Setup(f => f.WriteAllTextAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
                .Callback<string, string, CancellationToken>((path, text, _) => fileContent = text);
